...
function TypeProduitAvitaillementFormModal({
  isOpen,
  onClose,
  typeProduitAvitaillementId,
  isEditMode,
}) {
  ...
  useEffect(() => {
    if (isEditMode && typeProduitAvitaillementId) {
      const fetchTypeProduitAvitaillement = async () => {
        try {
          const typeProduitAvitaillementDoc = await getDoc(
            doc(db, "typeProduitAvitaillement", typeProduitAvitaillementId)
          );
          if (typeProduitAvitaillementDoc.exists()) {
            const data = typeProduitAvitaillementDoc.data();
            setFormData({
              nomProduit: data.nomProduit || "",
              active: data.active !== false,
            });
          } else {
            setError("Type de produit destiné à l'avitaillement non trouvé.");
          }
        } catch (err) {
      ...
      fetchTypeProduitAvitaillement();
    }
  }, [isEditMode, typeProduitAvitaillementId]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (
        !querySnapshot.empty &&
        (!isEditMode || querySnapshot.docs[0].id !== typeProduitAvitaillementId)
      ) {
        setError(
          "Ce type de produit destiné à l'avitaillement est déjà utilisé."
        );
        setLoading(false);
        return;
      }

      const typeProduitAvitaillementRef = doc(
        db,
        "typeProduitAvitaillement",
        isEditMode
          ? typeProduitAvitaillementId
          : doc(collection(db, "typeProduitAvitaillement")).id
      );
      await setDoc(
        typeProduitAvitaillementRef,
        {
              ...
              <div>
                <h2 className="text-lg font-bold text-white">
                  {isEditMode
                    ...
                    : "Nouveau type de produit destiné à l'avitaillement"}
                </h2>
                <p className="text-blue-200 text-xs">
                  {isEditMode
                    ? "Mettre à jour les informations"
                    : "Créer un nouveau type de produit destiné à l'avitaillement"}
                </p>
              </div>
            </div>
            <button
            ...
            <div className="space-y-4">
              <div className="space-y-1">
                ...
                <input
                  ...
                  placeholder="Saisir le type de produit destiné à l'avitaillement"
                  required
                />
              </div>
...
export default TypeProduitAvitaillementFormModal;
