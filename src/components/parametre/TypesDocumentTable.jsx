...
import TypeDocumentFormModal from "./TypeDocumentFormModal";

function TypesDocumentTable() {
  const [typesDocument, setTypesDocument] = useState([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [filterStatus, setFilterStatus] = useState("all");
  const [modalOpen, setModalOpen] = useState(false);
  const [editTypeDocumentId, setEditTypeDocumentId] = useState(null);

  const fetchTypesDocument = async () => {
    try {
      setLoading(true);
      const querySnapshot = await getDocs(collection(db, "typeDocument"));
      const typesDocumentList = querySnapshot.docs.map((doc) => ({
        id: doc.id,
        nomDocument: doc.data().nomDocument || "N/A",
        active: doc.data().active !== false,
      }));
      setTypesDocument(typesDocumentList);
    } catch (error) {
      console.error(
  ...
  const handleEdit = (typeDocumentId) => {
    setEditTypeDocumentId(typeDocumentId);
    setModalOpen(true);
  };

  const handleNewTypeDocument = () => {
    setEditTypeDocumentId(null);
    setModalOpen(true);
  };
  ...
  const filteredTypesDocument = typesDocument.filter((typeDocument) => {
    const matchesSearch = typeDocument.nomDocument
      .toLowerCase()
      .includes(searchTerm.toLowerCase());
    const matchesStatus =
      filterStatus === "all" ||
      (filterStatus === "active" && typeDocument.active) ||
      (filterStatus === "inactive" && !typeDocument.active);
    ...
    <div className="min-h-screen bg-gray-50 py-6">
      <div className="container mx-auto px-4">
          ...
          <div className="flex flex-wrap gap-4 mt-3 pt-3 border-t border-gray-200">
            <div className="flex items-center space-x-1">
              ...
              <span className="text-xs text-gray-600">
                {typesDocument.filter((d) => d.active).length} actifs
              </span>
            </div>
            <div className="flex items-center space-x-1">
              <div className="w-2 h-2 bg-red-500 rounded-full"></div>
              <span className="text-xs text-gray-600">
                {typesDocument.filter((d) => !d.active).length} inactifs
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          {filteredTypesDocument.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 text-sm">
                {searchTerm || filterStatus !== "all"
                  ...
                  : "Créez votre premier type de document"}
              </p>
              {!searchTerm && filterStatus === "all" && (
                <button
                  onClick={handleNewTypeDocument}
                ...
                >
                  <PlusCircle className="w-4 h-4" />
                  <span>Créer un type de document</span>
                </button>
              )}
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredTypesDocument.map((typeDocument) => (
                    <tr
                      key={typeDocument.id}
                    ...
                    >
                      <td className="py-3 px-4">
                        <div className="flex items-center space-x-2">
                          <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-medium bg-gradient-to-r from-blue-600 to-blue-700">
                            {typeDocument.nomDocument.charAt(0).toUpperCase()}
                          </div>
                      ...
                      <td className="py-3 px-4">
                        ...
                        >
                          ...
                          <span>
                            {typeDocument.active ? "Actif" : "Inactif"}
                          </span>
                        </span>
                      </td>
                      <td className="py-3 px-4">
                        <button
                          onClick={() => handleEdit(typeDocument.id)}
        ...
        {filteredTypesDocument.length > 0 && (
          <div className="mt-4 text-center text-gray-600 text-sm">
            Affichage de {filteredTypesDocument.length} type
            {filteredTypesDocument.length > 1 ? "s" : ""} de document
            {typesDocument.length !== filteredTypesDocument.length &&
              ` sur ${typesDocument.length} au total`}
          </div>
        )}

        <TypeDocumentFormModal
          isOpen={modalOpen}
          onClose={handleCloseModal}
          typeDocumentId={editTypeDocumentId}
          isEditMode={!!editTypeDocumentId}
        />
      </div>
    </div>
  );
...
export default TypesDocumentTable;
