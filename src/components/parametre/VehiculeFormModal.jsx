...
function VehiculeFormModal({ isOpen, onClose, vehiculeId, isEditMode }) {
      ...
      const fetchVehicule = async () => {
        try {
          ...
          if (vehiculeDoc.exists()) {
            ...
            setFormData({
              typeVehicule: data.typeVehicule || "Véhicule",
  ...
  }, [isEditMode, vehiculeId]);

  const handleChange = (e) => {
    const { name, value } = e.target;
    setFormData((prev) => ({
      ...prev,
      [name]: name === "idVehicule" ? value.toUpperCase() : value,
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Vérifier l'unicité de idVehicule
      const q = query(
        collection(db, "vehicules"),
        where("idVehicule", "==", formData.idVehicule)
      );
      const querySnapshot = await getDocs(q);
      if (
        !querySnapshot.empty &&
        (!isEditMode || querySnapshot.docs[0].id !== vehiculeId)
      ) {
        setError("Cet ID véhicule est déjà utilisé.");
        setLoading(false);
        return;
      }

      const vehiculeRef = doc(
        db,
        "vehicules",
        isEditMode ? vehiculeId : doc(collection(db, "vehicules")).id
      );
      await setDoc(
        vehiculeRef,
        {
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  onChange={handleChange}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 bg-gray-50 text-sm"
                  placeholder="Saisir l'ID du véhicule"
                  required
                />
              </div>
                ...
                <div className="flex items-center space-x-3 p-3 bg-gray-50 rounded-full border border-gray-200">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      {formData.active
                        ? "Le véhicule est utilisable dans l'application"
