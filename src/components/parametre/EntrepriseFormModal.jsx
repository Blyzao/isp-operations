...
function EntrepriseFormModal({ isOpen, onClose, entrepriseId, isEditMode }) {
  ...
  useEffect(() => {
    if (isEditMode && entrepriseId) {
      const fetchEntreprise = async () => {
        try {
          const entrepriseDoc = await getDoc(doc(db, "entreprise", entrepriseId));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const q = query(
        ...
        where("nomEntreprise", "==", formData.nomEntreprise)
      );
      const querySnapshot = await getDocs(q);
      if (
        !querySnapshot.empty &&
        (!isEditMode || querySnapshot.docs[0].id !== entrepriseId)
      ) {
        setError("Ce nom d'entreprise est déjà utilisé.");
        setLoading(false);
        return;
      }
      ...
      const entrepriseRef = doc(
        db,
        "entreprise",
        isEditMode ? entrepriseId : doc(collection(db, "entreprise")).id
      );
      await setDoc(
        entrepriseRef,
        {
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1">
                <label className="flex items-center space-x-1 text-xs font-medium text-gray-700">
                  ...
                  <span>Statut de l'entreprise</span>
                </label>
                <div className="flex items-center space-x-3 p-3 bg-gray-50 rounded-full border border-gray-200">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      {formData.active
                        ? "L'entreprise est utilisable dans l'application"
