...
function PersonnelsTable() {
  ...
  const handleEdit = (personnelId) => {
    setEditPersonnelId(personnelId);
    setModalOpen(true);
  };
      ...
      reader.onload = async (e) => {
        try {
          ...
          for (const item of json) {
            if (!item.matricule || !item.nomPrenom) {
              errors.push(
            ...
            if (!querySnapshot.empty) {
              errors.push(`Matricule déjà utilisé : ${item.matricule}`);
              continue;
            }
            ...
            try {
              await setDoc(doc(collection(db, "personnels")), personnelData);
            } catch (err) {
              errors.push(
                `Erreur lors de l'importation de ${item.matricule}: ${err.message}`
              );
            }
          }
  ...
  const filteredPersonnels = personnels.filter((personnel) => {
    const matchesSearch =
      ...
      personnel.nomPrenom.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesStatus =
      filterStatus === "all" ||
      (filterStatus === "active" && personnel.active) ||
      (filterStatus === "inactive" && !personnel.active);
      ...
      <div className="container mx-auto px-4">
          ...
          <div className="flex flex-wrap gap-4 mt-3 pt-3 border-t border-gray-200">
            <div className="flex items-center space-x-1">
              ...
              <span className="text-xs text-gray-600">
                {personnels.filter((p) => p.active).length} actifs
              </span>
            </div>
            <div className="flex items-center space-x-1">
              <div className="w-2 h-2 bg-red-500 rounded-full"></div>
              <span className="text-xs text-gray-600">
                {personnels.filter((p) => !p.active).length} inactifs
              </span>
            </div>
          </div>
          {importError && (
                    ...
                    >
                      <td className="py-3 px-4">
                        <div className="flex items-center space-x-2">
                          <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-medium bg-gradient-to-r from-blue-600 to-blue-700">
                            {personnel.nomPrenom.charAt(0).toUpperCase()}
                          </div>
                      ...
                      <td className="py-3 px-4">
                        ...
                        >
                          ...
                          <span>{personnel.active ? "Actif" : "Inactif"}</span>
                        </span>
                      </td>
                      <td className="py-3 px-4">
                        <button
        ...
        {filteredPersonnels.length > 0 && (
          <div className="mt-4 text-center text-gray-600 text-sm">
            Affichage de {filteredPersonnels.length} personnel
            {filteredPersonnels.length > 1 ? "s" : ""}
            {personnels.length !== filteredPersonnels.length &&
              ` sur ${personnels.length} au total`}
          </div>
        )}
