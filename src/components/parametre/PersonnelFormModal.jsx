...
function PersonnelFormModal({ isOpen, onClose, personnelId, isEditMode }) {
  ...
  useEffect(() => {
    if (isEditMode && personnelId) {
      const fetchPersonnel = async () => {
        try {
          const personnelDoc = await getDoc(doc(db, "personnels", personnelId));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const q = query(
        ...
        where("matricule", "==", formData.matricule)
      );
      const querySnapshot = await getDocs(q);
      if (
        !querySnapshot.empty &&
        (!isEditMode || querySnapshot.docs[0].id !== personnelId)
      ) {
        setError("Ce matricule est déjà utilisé.");
        setLoading(false);
        return;
      }
      ...
      const personnelRef = doc(
        db,
        "personnels",
        isEditMode ? personnelId : doc(collection(db, "personnels")).id
      );
      await setDoc(
        personnelRef,
        {
          matricule: formData.matricule,
          nomPrenom: formData.nomPrenom.toUpperCase(),
            ...
            <div className="flex items-center space-x-2">
              ...
              <div>
                <h2 className="text-lg font-bold text-white">
                  {isEditMode ? "Modifier le personnel" : "Nouveau personnel"}
                </h2>
                ...
                <div className="flex items-center space-x-3 p-3 bg-gray-50 rounded-full border border-gray-200">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      {formData.active
                        ? "Le personnel est utilisable dans l'application"
                        : "Le personnel est désactivé"}
                    </p>
                  </div>
                </div>
              </div>
