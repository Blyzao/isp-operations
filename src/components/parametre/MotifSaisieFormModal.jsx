...
function MotifSaisieFormModal({ isOpen, onClose, motifSaisieId, isEditMode }) {
  ...
  useEffect(() => {
    if (isEditMode && motifSaisieId) {
      const fetchMotifSaisie = async () => {
        try {
          const motifSaisieDoc = await getDoc(
            doc(db, "motifSaisie", motifSaisieId)
          );
          if (motifSaisieDoc.exists()) {
            const data = motifSaisieDoc.data();
            setFormData({
              motif: data.motif || "",
              active: data.active !== false,
            });
          } else {
            setError("Motif de saisie non trouvé.");
          }
        } catch (err) {
  ...
  const formatMotif = (motif) => {
    if (!motif) return "";
    return motif.charAt(0).toUpperCase() + motif.slice(1).toLowerCase();
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const formattedMotif = formatMotif(formData.motif);

    try {
      // Vérifier l'unicité de motif
      const q = query(
        collection(db, "motifSaisie"),
        where("motif", "==", formattedMotif)
      );
      const querySnapshot = await getDocs(q);
      if (
        !querySnapshot.empty &&
        (!isEditMode || querySnapshot.docs[0].id !== motifSaisieId)
      ) {
        setError("Ce motif de saisie est déjà utilisé.");
        setLoading(false);
        return;
      }

      const motifSaisieRef = doc(
        db,
        "motifSaisie",
        isEditMode ? motifSaisieId : doc(collection(db, "motifSaisie")).id
      );
      await setDoc(
        motifSaisieRef,
        {
          motif: formattedMotif,
            ...
            <div className="flex items-center space-x-2">
              ...
              <div>
                <h2 className="text-lg font-bold text-white">
                  {isEditMode
                    ? "Modifier le motif de saisie"
                    : "Nouveau motif de saisie"}
                </h2>
                <p className="text-blue-200 text-xs">
                  {isEditMode
                    ? "Mettre à jour les informations"
                    : "Créer un nouveau motif de saisie"}
                </p>
              </div>
            </div>
            <button
            ...
            <div className="space-y-4">
              <div className="space-y-1">
                ...
                <input
                  ...
                  placeholder="Saisir le motif de saisie"
                  required
                />
              </div>
              ...
              <div className="space-y-1">
                <label className="flex items-center space-x-1 text-xs font-medium text-gray-700">
                  ...
                  <span>Statut du motif de saisie</span>
                </label>
                <div className="flex items-center space-x-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                  ...
                  <div className="flex-1">
                    ...
                    >
                      {formData.active
                        ? "Motif de saisie actif"
                        : "Motif de saisie désactivé"}
                    </span>
                    <p className="text-xs text-gray-500 mt-1">
                      {formData.active
                        ? "Le motif de saisie est utilisable dans l'application"
                        : "Le motif de saisie est désactivé"}
                    </p>
                  </div>
                </div>
              </div>
...
export default MotifSaisieFormModal;
