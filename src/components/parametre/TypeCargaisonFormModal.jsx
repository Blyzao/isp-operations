...
function TypeCargaisonFormModal({
  isOpen,
  onClose,
  typeCargaisonId,
  isEditMode,
  ...
  useEffect(() => {
    if (isEditMode && typeCargaisonId) {
      const fetchTypeCargaison = async () => {
        try {
          const typeCargaisonDoc = await getDoc(
            doc(db, "typeCargaison", typeCargaisonId)
          );
          if (typeCargaisonDoc.exists()) {
            const data = typeCargaisonDoc.data();
            setFormData({
              nomCargaison: data.nomCargaison || "",
              active: data.active !== false,
            });
          } else {
            setError("Type de cargaison non trouvé.");
          }
        } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    const formattedNomCargaison = formatNomCargaison(formData.nomCargaison);

    try {
      // Vérifier l'unicité de nomCargaison
      const q = query(
        collection(db, "typeCargaison"),
        where("nomCargaison", "==", formattedNomCargaison)
      );
      const querySnapshot = await getDocs(q);
      if (
        !querySnapshot.empty &&
        (!isEditMode || querySnapshot.docs[0].id !== typeCargaisonId)
      ) {
        setError("Ce type de cargaison est déjà utilisé.");
        setLoading(false);
        return;
      }

      const typeCargaisonRef = doc(
        db,
        "typeCargaison",
        isEditMode ? typeCargaisonId : doc(collection(db, "typeCargaison")).id
      );
      await setDoc(
        typeCargaisonRef,
        {
          nomCargaison: formattedNomCargaison,
            ...
            <div className="flex items-center space-x-2">
              ...
              <div>
                <h2 className="text-lg font-bold text-white">
                  {isEditMode
                    ? "Modifier le type de cargaison"
                    : "Nouveau type de cargaison"}
                </h2>
                <p className="text-blue-200 text-xs">
                  {isEditMode
                    ? "Mettre à jour les informations"
                    : "Créer un nouveau type de cargaison"}
                </p>
              </div>
            </div>
            <button
            ...
            <div className="space-y-4">
              <div className="space-y-1">
                ...
                <input
                  ...
                  placeholder="Saisir le type de cargaison"
                  required
                />
              </div>
              ...
              <div className="space-y-1">
                <label className="flex items-center space-x-1 text-xs font-medium text-gray-700">
                  ...
                  <span>Statut du type de cargaison</span>
                </label>
                <div className="flex items-center space-x-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                  ...
                  <div className="flex-1">
                    ...
                    >
                      {formData.active
                        ? "Type de cargaison actif"
                        : "Type de cargaison désactivé"}
                    </span>
                    <p className="text-xs text-gray-500 mt-1">
                      {formData.active
                        ? "Le type de cargaison est utilisable dans l'application"
                        : "Le type de cargaison est désactivé"}
                    </p>
                  </div>
                </div>
              </div>
...
export default TypeCargaisonFormModal;
