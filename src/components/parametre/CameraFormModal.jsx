...
function CameraFormModal({ isOpen, onClose, cameraId, isEditMode }) {
  ...
  useEffect(() => {
    if (isEditMode && cameraId) {
      const fetchCamera = async () => {
        try {
          const cameraDoc = await getDoc(doc(db, "cameras", cameraId));
          if (cameraDoc.exists()) {
            const data = cameraDoc.data();
            setFormData({
              siteCamera: data.siteCamera || "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const q = query(
        ...
        where("idCamera", "==", formData.idCamera)
      );
      const querySnapshot = await getDocs(q);
      if (
        !querySnapshot.empty &&
        (!isEditMode || querySnapshot.docs[0].id !== cameraId)
      ) {
        setError("Cet ID caméra est déjà utilisé.");
        setLoading(false);
        return;
      }
      ...
      const cameraRef = doc(
        db,
        "cameras",
        isEditMode ? cameraId : doc(collection(db, "cameras")).id
      );
      await setDoc(
        cameraRef,
        {
          siteCamera: formData.siteCamera,
          idCamera: formData.idCamera,
            ...
            <div className="flex items-center space-x-2">
              ...
              <div>
                <h2 className="text-lg font-bold text-white">
                  {isEditMode ? "Modifier la caméra" : "Nouvelle caméra"}
                </h2>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                <label className="flex items-center space-x-1 text-xs font-medium text-gray-700">
                  ...
                  <span>Site de la caméra</span>
                </label>
                <input
                  type="text"
                  name="siteCamera"
                  value={formData.siteCamera}
                  ...
                  placeholder="Saisir le site de la caméra"
                  required
                />
              </div>
              ...
              <div className="space-y-1 col-span-2">
                ...
                <div className="flex items-center space-x-3 p-3 bg-gray-50 rounded-full border border-gray-200">
                  ...
                  <div className="flex-1">
                    ...
                    >
                      {formData.active ? "Caméra active" : "Caméra désactivée"}
                    </span>
                    <p className="text-xs text-gray-500 mt-1">
                      {formData.active
                        ? "La caméra est utilisable dans l'application"
              ...
              {error && (
                <div className="col-span-2 bg-red-50 border border-red-200 rounded-full p-3">
